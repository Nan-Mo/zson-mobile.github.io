...
describe('BackToTop', () => {
  ...
  it('should handle click event and scroll to the top with animation', () => {
    ...
    jest.useFakeTimers();
    wrapper.find('.zs-back-to-top').simulate('click');
    jest.runAllTimers();
  ...
  it('should append portal container to document body when component did mount', () => {
    ...
    const portalContainer = document.body.querySelector('.zs-back-to-top-container');
  ...
  it('should handle scroll and set visible to false if scrollTop greater than props.visibleDistance', () => {
    ...
    expect(wrapper.find('.zs-back-to-top').prop('style')).toEqual({
      display: 'none',
      position: 'fixed',
      bottom: 50,
      right: 50,
