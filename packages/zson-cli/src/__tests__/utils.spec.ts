...
describe('utils', () => {
  ...
  describe('#getCustomConfig', () => {
    it('should get default zson config file', () => {
      ...
      expect(actual).toEqual({ banner: 'zson' });
      expect(cwdSpy).toBeCalledTimes(1);
      expect(existsSyncMocked).toBeCalledWith(path.join(testDirPath, 'zson.config.js'));
    });
